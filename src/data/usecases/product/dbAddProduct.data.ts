...
export class DbAddProduct implements IAddProduct {
  ...
  async add(data: IAddProductDTO): Promise<IAddProductResult> {
    const company = await this.findUserIdRepository.findUserId(
      Number(data.user)
    )
    ...
    const productName = await this.findByProductNameRepository.findName(
      data.name
    )

    if (productName)
      return { error: 'Este nome já está em uso, escolha outro.' }
